...
document.querySelector('.topics-grid').addEventListener('click', (event) => {
  ...
  if (title === 'Twitter PNG') {
    ...
    newWindow.document.write(`
      ${svgPath}
      <button onclick="navigator.clipboard.writeText(\`${svgPath}\`).then(() => alert('SVG Path copied!'))">Copy SVG Path</button>
    `);
    newWindow.document.close();
    ...
    const newWindow = window.open('', '_blank');
    newWindow.document.write(`
      <canvas id="dynamicCanvas" width="512" height="512" style="display: block; margin: auto; border: 1px solid #000;"></canvas>
      <button onclick="navigator.clipboard.writeText(\`${canvasCode}\`).then(() => alert('Canvas Path copied!'))">Copy Canvas Path</button>
      <script>
